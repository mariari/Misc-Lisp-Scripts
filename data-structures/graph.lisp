             ...
             (cond ((null node-list) nil)
                   ((or (null node)
                    ...
                    (let ((node (car node-list)))
                      (rec (car node) (cdr node-list) (caddr node) seen (cadr node))))
                   ((funcall key find node) (cons find path))
                   (t (let* ((new-nodes (reduce (lambda (acc x) (cons (list x (cons node path) (1- lim)) acc))
                                                (remove-if (lambda (x) (member x seen)) ; O(n) :(
                                                           (hash-keys (get-node node graph)))
                                                :initial-value node-list))
                             (new-node (car new-nodes)))
                        (rec (car new-node) (cdr new-nodes) (caddr new-node) (push node seen) (cadr new-node)))))))
    (reverse (rec start (list (list start nil 0)) limit '() '()))))
