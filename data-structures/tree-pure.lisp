...
(defun tree!-insert (tree val)
  (cond ((null (tree!-left tree))  (make-tree! :val   (tree!-val tree)
                                               :left  (make-tree! :val val)))
        ((null (tree!-right tree)) (make-tree! :val   (tree!-val tree)
                                               :left  (tree!-left tree)
                                               :right (make-tree! :val val)))
        ((= (random 2) 1) (make-tree! :val   (tree!-val tree)
                                      :left  (tree!-insert (tree!-left tree) val)
                                      :right (tree!-right tree)))
        (t (make-tree! :val   (tree!-val tree)
                       :left  (tree!-left tree)
